...
def parse_matchstate(ms, p):
    ...
    bc = []
    print cards_spl
    np_board = np.zeros(5, dtype=np.int8)
    for b in board:
        while len(b) >= 2:
            if b[:2] == "10":
    ...
    np_hand = np.array(self_hand, dtype=np.int8)
    hand_i = ct.ctoi2[self_hand[0], self_hand[1]]
    ...
    action_odds = (r,c,f)
    if hi.cur_us.contents.gamestate == 3:
        print "OLD_ODDS", action_odds
        
        dic_key = (bet_path, tuple(np_board), tuple(np_hand))
        #if dic_key in solutions_dic:
        #act_ev = solutions_dic[dic_key]
        solutions_dic = {}
        
        act_ev = p.get_ev_for_hand(bet_path, np_board, np_hand, solutions_dic)
        normal_ev = 0.0
        normal_ev += action_odds[0]*act_ev[2][0]
        normal_ev += action_odds[1]*act_ev[2][1]
        normal_ev += action_odds[2]*act_ev[2][2]
        expect_edge += act_ev[0]-normal_ev
        print "EXPECT EDGE", expect_edge
        print solutions_dic
        action_odds = [0.0,0.0,0.0]
        print act_ev
        try:
            action_odds[act_ev[1]] = 1.0
        except:
            pdb.set_trace()
