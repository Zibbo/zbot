...
board_combo_types = np.fromfile(sys.argv[1], dtype=np.int16).reshape((8192, 2))
...
def dist_func2(types, new_type, diffs):
    ...
    if len(new_type) == 3:
        ...
        diffs[:] = b_diffs[1][types[:,0],new_type[0]]/max_diff[1]+b_diffs[2][types[:,1],new_type[1]]/max_diff[2]
...
slots = np.zeros((22100, 52), dtype=np.int16)
new_diffs = np.zeros(8192, dtype=np.float32)
        ...
        while hi.board[2] >= 0:
            ...
            while hi.board[3] >= 0:
                if hi.board[3] == hi.board[0] or hi.board[3] == hi.board[1] or hi.board[3] == hi.board[2]:
                    ...
                    lzp.set_st_from_hand_info(C.pointer(zp.i), C.pointer(hi), pub_types.ctypes.data_as(C.POINTER(C.c_int16)),priv_types.ctypes.data_as(C.POINTER(C.c_int16)))
                    t = np.array((pub_types[9],pub_types[8]), dtype=np.int16)
                    #dist_func2(board_combo_types, t, new_diffs)
                    #s = new_diffs.argmin()
                            
                    s2 = lzp.get_board_combo_slot(C.pointer(zp.i), max_diff.ctypes.data_as(C.POINTER(C.c_float)), board_combo_types.ctypes.data_as(C.POINTER(C.c_int16)), t.ctypes.data_as(C.POINTER(C.c_int16)), 2, 8192)
                    #print s, s2
                    #print s, s2
                    slots[flop_i, hi.board[3]] = s2
                    hi.board[3]-=1
            hi.board[2]-=1
        hi.board[1]-=1
...
slots.tofile("board_combo_slots_gs2.slots")
